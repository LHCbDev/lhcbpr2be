...
class JobResultsService:
    ...
    def get_results_per_attr_per_version(self, context, only_numeric=True):
        ...
        if 'attr_filter' in context and context['attr_filter']:
            attrs_ids = attrs_ids.filter(
                name__icontains=context['attr_filter'])
        if only_numeric:
        ...
        if 'app' in context and context['app']:
            queryset = queryset.filter(
                job__job_description__application_version__application__id__in=context['app'])
        if 'options' in context and context['options']:
            queryset = queryset.filter(
                job__job_description__option__id__in=context['options'])
        ...
        queryset = queryset.order_by(
            'attr__id', 'job__job_description__application_version__version')
        ...
        logger.info(
            'Looping over Queryset: {0} elements'.format(len(queryset)))
        i = 1
        ...
        for item in queryset:
            ...
            if item['resultfloat__data'] is not None:
                ...
                results[current_result_index]['values'][current_version_index][
                    'results'].append(item['resultinteger__data'])
